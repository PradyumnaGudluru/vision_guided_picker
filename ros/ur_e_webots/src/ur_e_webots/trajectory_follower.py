...
def interp_cubic(p0, p1, t_abs):
    ...
    for i in range(len(p0.positions)):
        ...
        q_dot[i] = b + 2 * c * t + 3 * d * t**2
        q_ddot[i] = 2 * c + 6 * d * t
    return JointTrajectoryPoint(positions=q, velocities=q_dot, accelerations=q_ddot,
                                time_from_start=rospy.Duration(t_abs))


def sample_trajectory(trajectory, t):
    """Return (q, q_dot, q_ddot) for sampling the JointTrajectory at time t,
...
class TrajectoryFollower(object):
    ...
    def __init__(self, robot, joint_state_publisher, joint_prefix, node_name,
                 goal_time_tolerance=None):
        self.robot = robot
        # what is this joint_prefix?
        self.joint_prefix = joint_prefix
        self.prefixed_joint_names = [s + self.joint_prefix for s in TrajectoryFollower.joint_names]
        self.joint_state_publisher = joint_state_publisher
        self.time_step = int(robot.getBasicTimeStep())
        self.motors = []
        self.sensors = []
        self.trajectory_t0 = None

        for name in TrajectoryFollower.joint_names:
            self.motors.append(robot.getDevice(name))
            self.sensors.append(robot.getDevice(name + '_sensor'))
            self.sensors[-1].enable(self.time_step)

        self.goal_handle = None
        self.trajectory = None
        self.joint_goal_tolerances = [0.05, 0.05, 0.05, 0.05, 0.05, 0.05]
        self.server = actionlib.ActionServer(node_name + "follow_joint_trajectory",
                                             FollowJointTrajectoryAction,
                                             self.on_goal, self.on_cancel, auto_start=False)

    def init_trajectory(self):
        """Initialize a new target trajectory."""
        state = self.joint_state_publisher.last_joint_states
        self.trajectory_t0 = self.robot.getTime()
        self.trajectory = JointTrajectory()
        self.trajectory.joint_names = self.prefixed_joint_names
    ...
    def on_goal(self, goal_handle):
        ...
        if set(goal_handle.get_goal().trajectory.joint_names) != set(self.prefixed_joint_names):
        ...
        if not trajectory_is_finite(goal_handle.get_goal().trajectory):
            rospy.logerr("Received a goal with infinities or NaNs")
            goal_handle.set_rejected(text="Received a goal with infinities or NaNs")
            return
        ...
        reorder_trajectory_joints(goal_handle.get_goal().trajectory, self.prefixed_joint_names)
        ...
        point0.time_from_start = rospy.Duration(0)
    ...
    def on_cancel(self, goal_handle):
        ...
        if goal_handle == self.goal_handle:
            # stop the motors
            for i in range(len(TrajectoryFollower.joint_names)):
